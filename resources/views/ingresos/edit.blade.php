<x-app-layout>
    <h1 class="text-2xl font-semibold mb-6 text-gray-800">Editar Ingreso #{{ $ingreso->idingreso }}</h1>

    <form action="{{ route('ingresos.update', $ingreso->idingreso) }}" method="POST" class="space-y-6 bg-white p-8 rounded-lg shadow-lg max-w-lg mx-auto">
        @csrf
        @method('PUT')
        ...
        <div>
            ...
            <textarea name="descripcion" id="descripcion" rows="4"
                class="w-full border border-gray-300 rounded-md px-4 py-3 focus:outline-none focus:ring-2 focus:ring-green-500 focus:border-transparent transition">{{ old('descripcion', $ingreso->descripcion) }}</textarea>
            @error('descripcion')
                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
            @enderror
        </div>
